...
def get_dataset(dataroot_pc, npoints, svpoints, category):
    tr_dataset = ShapeNet(dataroot=dataroot_pc, split='train', npoints=npoints, svpoints=svpoints, category=category)
    te_dataset = ShapeNet(dataroot=dataroot_pc, split='test', npoints=npoints, svpoints=svpoints, category=category)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    ''' data '''
    train_dataset, _ = get_dataset(opt.dataroot_pc, opt.npoints, opt.svpoints, opt.category)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{opt.niter}", unit="batch")):
            randind_num = np.random.choice(8, size=(num_views,))  # 8 views
            randind_MDM = np.random.choice(8, size=(MDM_views,))  # 8 views

            x = data['complete_point'].transpose(1, 2).cuda(gpu)  # [10, 3, 16384]
            sv_x_list = []
            sv_MDM_list = []
            for j in range(num_views):
                sv_x_list.append(data['partial_point'][:, randind_num[j]].transpose(1, 2).cuda(gpu))  # [10, 3, 2048]
                sv_x_list[j] = torch.cat((sv_x_list[j], x[:, :, opt.svpoints:]), dim=-1).cuda(gpu)
            for j in range(MDM_views):
                sv_MDM_list.append(data['partial_point'][:, randind_MDM[j]].transpose(1, 2).cuda(gpu))  # [10, 3, 2048]

            noises_batch = noises_init[data['idx']].transpose(1, 2).cuda(gpu)
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            model.eval()
            m, s = train_dataset.mean.reshape(1, -1).cuda(gpu), train_dataset.std.reshape(1, -1).cuda(gpu)

            with torch.no_grad():
                x_gen_eval = model.gen_samples(
                    sv_x_list[0][:, :, opt.svpoints], sv_x_list[0][:, :, opt.svpoints:].shape, sv_x_list[0].device,
            ...
            export_to_pc_batch('%s/epoch_%03d_samples_eval' % (outf_syn, epoch),
                               (x_gen_eval.transpose(1, 2) * s.cpu() + m.cpu()).numpy() * 3)

            export_to_pc_batch('%s/epoch_%03d_ground_truth' % (outf_syn, epoch),
                               (sv_x_list[0].transpose(1, 2).detach().cpu() * s.cpu() + m.cpu()).numpy() * 3)

            export_to_pc_batch('%s/epoch_%03d_partial' % (outf_syn, epoch),
                               (sv_x_list[0][:, :, opt.svpoints].transpose(1, 2).detach().cpu() * s.cpu() + m.cpu()).numpy() * 3)
...
def main():
    ...
    noises_init = torch.randn(len(train_dataset), opt.npoints - opt.svpoints, opt.nc).cuda()
...
def parse_args():
    ...
    parser.add_argument('--saveIter', default=200, help='unit: epoch')
    parser.add_argument('--diagIter', default=200, help='unit: epoch')
    parser.add_argument('--vizIter', default=200, help='unit: epoch')
    parser.add_argument('--print_freq', default=200, help='unit: iter')
