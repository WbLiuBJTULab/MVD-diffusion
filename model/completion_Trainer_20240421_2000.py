...
class GaussianDiffusion:
    ...
    def p_sample_loop(self, partial_x, denoise_fn, shape, device,
        ...
        B, D, N = partial_x.shape
        bs, _, _ = shape
        partial_views = B // bs
        cat_partial_x = partial_x[:bs]

        if partial_views < self.num_views:
            img_t = torch.cat([cat_partial_x] * self.num_views, dim=-1)
        else:
            img_t = torch.cat([partial_x[i:i+bs] for i in range(0, self.num_views * bs, bs)], dim=-1)

        if self.MDM_views:
            MDM_noise_shape = torch.cat([cat_partial_x] * self.MDM_views, dim=-1).shape
...
class Modeltrainer_c(nn.Module):
    ...
    def gen_samples(self, partial_x, shape, device, noise_fn=torch.randn,
        ...
        return self.diffusion.p_sample_loop(partial_x=partial_x, denoise_fn=self._denoise, shape=shape, device=device, noise_fn=noise_fn,
                                            clip_denoised=clip_denoised,
