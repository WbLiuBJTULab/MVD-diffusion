...
from model.pvcnn_completion_20240624_1500 import PVCNN2_VAE
...
class GaussianDiffusion:
    ...
    def _vb_terms_bpd(self, denoise_fn, data_start, data_t, t, clip_denoised: bool, return_pred_xstart: bool, noise_fn=torch.randn):
        ...
        #这里原来有问题，可以替换为
        # init_in = [noise_fn(size=[B , D, N * self.num_views], dtype=torch.float, device='cuda')]
        # print(data_t_in[0].shape) [18, 3, 200]
        # print(init_in[0].shape) [18, 3, 600]
        data_t_in = torch.cat(data_t_in + data_t_in + [data_t], dim=-1)
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None):
        ...
        data_in = torch.cat(data_in + data_in + [data_t], dim=-1)
        # 调试代码
