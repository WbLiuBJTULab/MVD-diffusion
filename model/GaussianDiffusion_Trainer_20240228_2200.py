...
from model.pvcnn_completion_20240228_2200 import PVCNN2Base
from model.Mutli_latent_old import PVCNN2Base_latent
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, denoise_fn, data, t, clip_denoised: bool, return_pred_xstart: bool):
        ...
        # _4-2修改位置
        # _核心代码，将数据读入模型的位置
        model_output = denoise_fn(data, t, self.sv_points, self.num_views)[:, :, self.num_views * self.sv_points:]
        ...
        if self.model_mean_type == 'eps':
            # _修改位置
            x_recon = self._predict_xstart_from_eps(data[:, :,self.num_views * self.sv_points:], t=t, eps=model_output)
            # _必须修改位置，此处对于eps的计算问题很大
            # _修改位置，此处先硬性写成使用主视角
            # _修改位置
            model_mean, _, _ = self.q_posterior_mean_variance(x_start=x_recon,
                                                              x_t=data[:, :, self.num_views * self.sv_points:], t=t)
        else:
    ...
    def p_sample(self, denoise_fn, data, t, noise_fn, clip_denoised=False, return_pred_xstart=False):
        ...
        #_修改位置
        sample = torch.cat([data[:, :, :self.num_views * self.sv_points], sample], dim=-1)
        #调试代码
        '''
        print('sample___')
    ...
    def p_sample_loop(self, partial_x, denoise_fn, shape, device,
        ...
        img_t = torch.cat([partial_x,
                           partial_x,
                           partial_x,
                           noise_fn(size=shape, dtype=torch.float, device=device)], dim=-1)
        ...
        assert img_t[:, :, self.num_views * self.sv_points:].shape == shape
        #img_t[:,:,:].shape [80, 3, 2448]
        return img_t
    ...
    def _vb_terms_bpd(self, denoise_fn, data_start, data_t, t, clip_denoised: bool, return_pred_xstart: bool):
        ...
        #调试代码
        '''
        print(data_start[0].shape)#[8, 3, 2048]
        print(data_t.shape)#[8, 3, 2048]
        '''
        #_修改位置，此处先硬性写成使用主视角
        true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(
            x_start=data_start[0][:,:,self.sv_points:], x_t=data_t[:,:,self.sv_points:], t=t)

        data_t_in = torch.cat([data_start[0][:, :, :self.sv_points],
                               data_start[1][:, :, :self.sv_points],
                               data_start[2][:, :, :self.sv_points],
                               data_t[:,:,self.sv_points:]], dim=-1)
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None):
        ...
        data_in = torch.cat(data_in + [data_t], dim=-1)
        # 调试代码
        '''print(data_in.shape)#[8, 3, 2448]'''
        ...
        if self.loss_type == 'mse':
            ...
            # 调试代码
            '''
            print(self.sv_points)#200
            print(eps_recon.shape)#[6, 3, 1848]
            print(noise.shape)#[6, 3, 1848]
            print(data_start[0].shape)#[6, 3, 2048]
            '''
    ...
    def calc_bpd_loop(self, denoise_fn, x_start, clip_denoised=True):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(T)):
                ...
                data_t = torch.cat([x_start[0][:, :, :self.sv_points], self.q_sample(x_start=x_start[0][:, :, self.sv_points:], t=t_b)], dim=-1)
...
class PVCNN2_latent(PVCNN2Base_latent):
    sa_blocks = [
        ((32, 2, 32), (1024, 0.1, 32, (32, 64))),
        ((64, 3, 16), (256, 0.2, 32, (64, 128))),
        ((128, 3, 8), (64, 0.4, 32, (128, 256))),
...
class Modeltrainer(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.latent_model = PVCNN2_latent(num_classes=args.nc, sv_points=args.svpoints, embed_dim=args.embed_dim,
    ...
    # _4-2修改位置
    def _denoise(self, data, t, sv_points, num_views):
        # _4-2修改位置
        B, D, N = data.shape
        assert data.dtype == torch.float
        assert t.shape == torch.Size([B]) and t.dtype == torch.int64
        partial_cloud_slice = data[:, :, : num_views* sv_points]
        noised_data_slice = data[:, :, num_views * sv_points:]
        mutli_latent = self.latent_model(pratial_cloud = partial_cloud_slice, t = t,
                                         data_points = sv_points, num_views = num_views)

        #print(mutli_latent.shape)# [8, 64, 600]

        denoise_out = self.model(inputs = noised_data_slice, mutli_latent = mutli_latent, t = t,
                         data_points = sv_points, num_views = num_views)

        #print(denoise_out.shape)# [8, 3, 2448]
