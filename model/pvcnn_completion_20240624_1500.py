...
from model.vae_pointnet_20240421 import VAE_tofeatrue
...
class PVCNN2_VAE(nn.Module):

    def __init__(self, num_classes, sv_points, embed_dim, num_views, MDM_views, MDM_multiplier, latent_multiplier, use_att, dropout=0.1,
        ...
        self.global_att = None if not use_att else Attention(channels_sa_features, num_groups= 8, D=1)
        ...
        # GenRe use *2
        # PCN use *20
        self.latent_featrue = VAE_tofeatrue(latent_dims=self.MDM_views * self.latent_multiplier, sv_points = int( self.MDM_multiplier * self.sv_points))
        #self.latent_featrue = VAE_tofeatrue(latent_dims=self.MDM_views * 2, sv_points = int(self.sv_points/8))
    ...
    def forward(self, input, t , data_points = 2048, num_views = 3, MDM_views = 3):
        # print(input.shape)  # [7, 3, 26624]
        # noised_data_input = torch.cat([input[:, :, :data_points], input[:, :, num_views * data_points:]],dim=-1)
        noised_data_input = torch.cat([input[:, :, :num_views * data_points], input[:, :, (num_views + MDM_views) * data_points:]], dim=-1)
        temb = self.embedf(self.get_timestep_embedding(t, noised_data_input.device))[:, :, None].expand(-1, -1, noised_data_input.shape[-1])

        # inputs : [B, in_channels + S, N]
        coords, features = noised_data_input[:, :3, :].contiguous(), noised_data_input
        ...
        in_features_list[0] = noised_data_input[:, 3:, :].contiguous()
        ...
        # _调试代码
        # print(features.shape)# [7, 64, 20480]
        partial_cloud_input = input[:, :, num_views * data_points : (num_views + MDM_views) * data_points]
        z_feature = self.latent_featrue(partial_cloud_input, int(self.MDM_multiplier * data_points))
        #print(z_feature.shape)# [7, 64, 256]
        fused_features = torch.cat([z_feature, features], dim=-1)
        # print(combined_features.shape)#[4, 128, 2048]
        # print(fuse_features.shape)#[4, 64, 2048]
