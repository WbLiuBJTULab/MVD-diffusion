...
def get_dataset(opt):
    tr_dataset = ShapeNet(dataroot=opt.dataroot_pc, split='train', npoints=opt.npoints, svpoints=opt.svpoints, category=opt.category,
                          num_views=opt.num_views, MDM_views=opt.MDM_views)
    te_dataset = ShapeNet(dataroot=opt.dataroot_pc, split='test', npoints=opt.npoints, svpoints=opt.svpoints, category=opt.category,
                          num_views=opt.num_views, MDM_views=opt.MDM_views)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            model.eval()
            m, s = train_dataset.mean.reshape(1, -1), train_dataset.std.reshape(1, -1)
            ...
            export_to_pc_batch('%s/epoch_%03d_ground_truth' % (outf_syn, epoch),
                               (sv_x_list[0].transpose(1, 2).detach().cpu()*s+m).numpy()*3)

            export_to_pc_batch('%s/epoch_%03d_partial' % (outf_syn, epoch),
                               (sv_x_list[0][:,:,:opt.svpoints].transpose(1, 2).detach().cpu()*s+m).numpy()*3)
...
def main():
    ...
    #_后续需修改位置，由于设置第0个为主视角，
    # 且利用其他两个视角数据做约束，所以噪声第二维应是opt.npoints-opt.svpoints+3*opt.svpoints
