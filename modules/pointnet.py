...
__all__ = ['PointNetAModule', 'PointNetSAModule', 'PointNetSAModule_og', 'PointNetFPModule', 'PointNetFPModule_og']
...
# _修改位置
class PointNetSAModule_og(nn.Module):
    def __init__(self, num_centers, radius, num_neighbors, in_channels, out_channels, include_coordinates=True):
        super().__init__()
        if not isinstance(radius, (list, tuple)):
            radius = [radius]
...
# _修改位置
class PointNetFPModule_og(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.mlp = SharedMLP(in_channels=in_channels, out_channels=out_channels, dim=1)
