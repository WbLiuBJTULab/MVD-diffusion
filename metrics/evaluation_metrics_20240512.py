...
def EMD_CD(sample_pcs, ref_pcs, batch_size,  reduced=True):
    ...
    for b_start in iterator:
        ...
        # dl ,dr 相当于cd提供计算出的dist1和dist2
        fs = fscore(dl, dr)[0].cpu()
        fs_lst.append(fs)
        '''
        dl1 = torch.mean(torch.sqrt(dl), dim=1)
        dr1 = torch.mean(torch.sqrt(dr), dim=1)1
        cd1_lst.append((dl1 + dr1) / 2)
        '''
        dl2 = torch.mean(dl, dim=1)
        dr2 = torch.mean(dr, dim=1)
        cd2_lst.append(dl2 + dr2)

        dl1 = torch.sqrt(dl2)
        dr1 = torch.sqrt(dr2)
        cd1_lst.append((dl1 + dr1) / 2)
        #cd1_lst.append(dl.mean(dim=1) + dr.mean(dim=1))
        '''
        #这形状还会变？
        print(dl1.shape) # torch.Size([6])
        print(dr1.shape) # torch.Size([6])
        print(((dl1 + dr1) / 2).shape) # torch.Size([6])
        print((torch.sum( dl1 + dr1 )/2).shape) # torch.Size([])
        print(dl1) # tensor([0.0281, 0.0105, 0.0090, 0.0141, 0.0145, 0.0138], device='cuda:0')
        print(dr1) # tensor([0.0151, 0.0085, 0.0171, 0.0097, 0.0138, 0.0079], device='cuda:0')
        print(((dl1 + dr1) / 2)) # tensor([0.0216, 0.0095, 0.0131, 0.0119, 0.0142, 0.0109], device='cuda:0')
        print(torch.sum( dl1 + dr1 )/2)# tensor(0.0811, device='cuda:0')
        '''
        emd_batch = EMD(sample_batch.cuda(), ref_batch.cuda(), transpose=False)
        emd_lst.append(emd_batch)

    if reduced:
        cd1 = torch.cat(cd1_lst).mean()
        cd2 = torch.cat(cd2_lst).mean()
        emd = torch.cat(emd_lst).mean()
    else:
        cd1 = torch.cat(cd1_lst)
        cd2 = torch.cat(cd2_lst)
...
#New adding
def l2_cd(pcs1, pcs2):
    dist1, dist2, _, _ = cham3D(pcs1.cuda(), pcs2.cuda())
...
#New adding
def emd(pcs1, pcs2):
    dists = EMD(pcs1.cuda(), pcs2.cuda())
    return torch.sum(dists)

#New adding
def f_score(pcs1, pcs2):
    dists = fscore(pcs1, pcs2)[0].cpu()
    return torch.sum(dists)
